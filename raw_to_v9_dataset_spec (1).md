...
> 작성일: 2026-08-01
> 상태: **체인 검증 완료** — 각 단계의 재현/일치 여부를 데이터 비교로 확인
...
원자료 (데이터/open/)
  │
  ├─① 전처리 파이프라인        ── preprocess_for_team.py (0.6244 레시피)
  │     LDAPS/GFS NWP 피처 엔지니어링 + SCADA 기반 보정 + 시계열 피처
  │     → preprocessed_train/test_data.csv  (323열)
  │
  ├─② 팀 피처 15열 추가        ── 생성 코드 미확보 (팀원 산출물)
  │     ws_corr / pc_pred / pc_clean / ws_qm / bestlayer_vcubed × 3그룹
  │     → merged_train_v8.csv (338열) / merged_test_v8.csv (336열)
  │
  └─③ 신규 피처 120열 추가     ── engineer_new_features.py ✅ (재현 검증 완료)
...
| prefix | 의미(추정) | 개수 |
|---|---|---|
| `ws_corr_kpx_group_{1,2,3}` | 풍속-출력 상관 특성 | 3 |
| `pc_pred_kpx_group_{1,2,3}` | 파워커브 기반 예측값 | 3 |
| `pc_clean_kpx_group_{1,2,3}` | 파워커브 클린 마스크 | 3 |
| `ws_qm_kpx_group_{1,2,3}` | 풍속 quantile 매핑 | 3 |
| `bestlayer_vcubed_kpx_group_{1,2,3}` | 최적층 풍속³ | 3 |

- 배치: v8 컬럼 순서 pos 323~337 (전처리 컬럼 뒤, 그룹별 5개씩 1/2/3 순서)
- **구조 검증**: v8의 나머지 323열은 단계① 산출물과 kst_dtm 정렬 기준 값 100% 일치 (§3)

### 2.3 생성 코드 현황

- ❌ **이 저장소에는 15열을 만드는 코드가 없음** — 모든 스크립트/노트북은 이 컬럼들을 `TEAM` 세트로 묶어 **모델 입력에서 제외**하는 용도로만 참조 (예: `cv_*.py`, `colab_*.ipynb`의 `TEAM = sorted([c for c in ... if c.startswith(...)])`)
- → 팀원이 별도로 만들어 공유한 것으로 판단 (파일명 `teammate feature/` 폴더, 생성일 2026-07-09)
- **재현에 필요한 것**: 팀원 측의 해당 15열 생성 코드 (학교 서버 `v16_exp` 폴더 등) — 2차 평가 점수복원 규정상 요구됨
...
| v8 = 전처리 + 팀 15열 | v8 컬럼 집합 − 전처리 컬럼 집합 = 정확히 15개, pos 323~337에 배치 | 확인 |
...
```bash
# ① 전처리 (원자료 → 323열) — 저장소에 정본 존재
python scripts/preprocess_for_team.py
#   → preprocessed_train_data.csv, preprocessed_test_data.csv

# ② 팀 피처 15열 (323 → v8 338열) — ⚠️ 생성 코드 미확보 (팀원 서버 필요)
#    현재는 v8 CSV가 그대로 사용됨

# ③ 신규 피처 120열 (v8 → v9 458열) — 저장소에 정본 존재
python engineer_new_features.py
#   → teammate feature/merged_train_v9.csv, merged_test_v9.csv
#   ⚠️ 원본 스크립트는 teammate feature/에 덮어쓰므로, 재현 시 출력 경로를 바꾼 복사본 권장
```

---

## 7. 미해결 항목 (2차 평가 점수복원 대비)

| # | 항목 | 영향 | 필요 출처 |
|---|---|---|---|
| 1 | **팀 피처 15열 생성 코드 미확보** | 원자료→v8 체인 불완전 | 팀원 (학교 서버 `v16_exp` 폴더 전처리 스크립트) |
| 2 | v10 생성 과정 문서화 | v10 사용 시 재현 필요 | `scripts/fetch_openmeteo_weather.py` 참조 |
